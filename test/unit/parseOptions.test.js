...
test('parseOptions sets correct default options', () => {
  ...
  expect(options.managementApplication).toBe(`contentful.export/${version}`)
  ...
  expect(options.sourceManagementToken).toBe(managementToken)
  expect(options.sourceSpace).toBe(spaceId)
