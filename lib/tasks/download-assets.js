 ...
 * @param {string} options.fileName - The original filename of the asset.
 * @param {import('axios').AxiosInstance} options.httpClient - The HTTP client to use for downloading the asset.
 */
async function downloadAsset ({ url, directory, fileName, httpClient }) {
// handle urls without protocol
  ...
  const localFile = buildLocalFilePath(url, directory, fileName)
  ...
  return (ctx, task) => {
    ...
    return Promise.map(ctx.data.assets, (asset) => {
      ...
      return Promise.mapSeries(locales, (locale) => {
        ...
        let startingPromise = Promise.resolve({ url, directory: options.exportDir, fileName, httpClient })
        ...
        if (isEmbargoedAsset(url)) {
          ...
          startingPromise = signUrl(host, accessToken, spaceId, environmentId, url, expiresAtMs, httpClient)
            .then((signedUrl) => ({ url: signedUrl, directory: options.exportDir, fileName, httpClient }))
        }
