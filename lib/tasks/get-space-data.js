...
// When `experimentalHaltOnTimeout` is enabled this halts program execution if
// an entry/asset/content-type page request times out. Only entityNames listed
// here are timeout-guarded - add an entry here (and nowhere else) to extend
// the guard to another pagedGet() caller.
const ENTRY_AND_CONTENT_TYPE_FETCH_TIMEOUT_MS = 20_000
const ASSET_FETCH_TIMEOUT_MS = 30_000
const TIMEOUT_MS_BY_ENTITY = {
  'content types': ENTRY_AND_CONTENT_TYPE_FETCH_TIMEOUT_MS,
  entries: ENTRY_AND_CONTENT_TYPE_FETCH_TIMEOUT_MS,
  assets: ASSET_FETCH_TIMEOUT_MS
}
...
export default function getFullSourceSpace({
  ...
  return new Listr([
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        return pagedGet((query) => client.contentType.getMany({ spaceId, environmentId, query }), { entityName: 'content types', resource: 'contentType', method: 'getMany', spaceId, environmentId, haltOnTimeout: experimentalHaltOnTimeout })
          .then(extractItems)
          .then((items) => {
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        return pagedGet((query) => client.tag.getMany({ spaceId, environmentId, query }), { entityName: 'tags' })
          .then(extractItems)
          .then((items) => {
            ctx.data.tags = items
          })
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        return pagedGet((query) => client.locale.getMany({ spaceId, environmentId, query }), { entityName: 'locales' })
          .then(extractItems)
          .then((items) => {
 ...
 * @param options {{ query?: object, entityName?: string, resource?: string, method?: string, spaceId?: string, environmentId?: string, haltOnTimeout?: boolean }}
 */
function pagedGet(fetchFn, { query: userQuery = null, entityName, resource, method, spaceId, environmentId, haltOnTimeout } = {}, skip = 0, aggregatedResponse = null) {
  ...
  const timeoutMs = TIMEOUT_MS_BY_ENTITY[entityName]
  ...
  return pagePromise
    .then((response) => {
      ...
      return pagedGet(fetchFn, { query: userQuery, entityName, resource, method, spaceId, environmentId, haltOnTimeout }, skip + response.items.length, aggregatedResponse)
    })
    .catch((err) => {
      ...
      return pagedGet(fetchFn, { query: userQuery, entityName, resource, method, spaceId, environmentId, haltOnTimeout }, nextSkip, aggregatedResponse)
    })
}
