...
export default function getFullSourceSpace ({
  ...
  return new Listr([
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        // The webhook API does not support pagination query params (skip, limit, order)
        return pagedGet(() => {
          return client.webhook.getMany({ spaceId })
        })
          .then(extractItems)
          .then((items) => {
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        // The roles API does not support pagination query params (skip, limit, order)
        return pagedGet(() => {
          return client.role.getMany({ spaceId })
        })
          .then(extractItems)
          .then((items) => {
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.components = await cursorPagedGet({ client: plainClient, spaceId, environmentId, method: 'component.getMany' })
        } catch (err) {
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.experienceTemplates = await cursorPagedGet({ client: plainClient, spaceId, environmentId, method: 'experienceTemplate.getMany' })
        } catch (err) {
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.experienceFragments = await cursorPagedGet({ client: plainClient, spaceId, environmentId, method: 'experienceFragment.getMany' })
        } catch (err) {
...
function logPagingStatus (response, query, userLimit) {
  ...
  const imposedLimit = limit || query.limit
  const limitedTotal = userLimit ? Math.min(userLimit, total) : total
  const page = Math.ceil(query.skip / imposedLimit) + 1
