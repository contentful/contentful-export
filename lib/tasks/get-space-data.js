...
export default function getFullSourceSpace ({
  ...
  return new Listr([
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        // The webhook API does not support the 'order' query param
        return pagedGet((query) => {
          const { order: _order, ...webhookQuery } = query
          return client.webhook.getMany({ spaceId, query: webhookQuery })
        })
          .then(extractItems)
          .then((items) => {
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        // The roles API does not support the 'order' query param
        return pagedGet((query) => {
          const { order: _order, ...rolesQuery } = query
          return client.role.getMany({ spaceId, query: rolesQuery })
        })
          .then(extractItems)
          .then((items) => {
