...
export default function getFullSourceSpace ({
  ...
  return new Listr([
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.components = await cursorPagedGet({ client: plainClient, spaceId, environmentId, method: 'componentType.getMany' })
        } catch (err) {
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.experienceTemplates = await cursorPagedGet({ client: plainClient, spaceId, environmentId, method: 'template.getMany' })
        } catch (err) {
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.experienceFragments = await cursorPagedGet({ client: plainClient, spaceId, environmentId, method: 'fragment.getMany' })
        } catch (err) {
