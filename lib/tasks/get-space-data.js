      ...
      task: wrapTask((ctx, task) => {
        return pagedGet({source: ctx.environment, method: 'getTags'})
            ...
            .catch(e => {
              ...
              // not all spaces have tags enabled
              ctx.data.tags = []
              return
            })
      }),
      skip: (ctx) => {
        console.log(!skipContent && !ctx.data.entries[0].metadata)
        // We're using entries to determine if the feature is enabled
        // if you skip content, you won't get tags back either
        // this also covers the CDA case-- no metadata properties will be returned
        // so it skips exporting tags if using cdatoken
        return skipContent || !ctx.data.entries[0].metadata
      }
    },
    {
...
function pagedGet ({ source, method, skip = 0, aggregatedResponse = null, query = null }) {
  ...
  // workaround because tags API returns no tags if we send unsupported query params
  return source[method](method === 'getTags' ? '' : fullQuery)
    .then((response) => {
