  ...
  return new Listr([
    {
      ...
      task: wrapTask(async (ctx) => {
        ...
        const environment = await client.environment.get({ spaceId, environmentId })
        ctx.space = space
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.components = await cursorPagedGet({ client, spaceId, environmentId, method: 'component.getMany' })
        } catch (err) {
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.experienceTemplates = await cursorPagedGet({ client, spaceId, environmentId, method: 'experienceTemplate.getMany' })
        } catch (err) {
    ...
    },
    {
      ...
      task: wrapTask(async (ctx) => {
        try {
          ctx.data.experienceFragments = await cursorPagedGet({ client, spaceId, environmentId, method: 'experienceFragment.getMany' })
        } catch (err) {
...
async function cursorPagedGet ({ client, spaceId, environmentId, method }) {
  ...
  do {
    ...
    const response = await client[entity][operation]({
      spaceId,
      environmentId,
      query
    })
    ...
    pageNext = getPageNext(response.pages?.next)
  } while (pageNext)
...
function getPageNext (next) {
  ...
  try {
    const url = new URL(next, 'https://contentful.invalid')
    return url.searchParams.get('pageNext') || next
  } catch {
    return next
  }
}
...
async function pagedRolesGet ({ client, spaceId }) {
  ...
  if (response.pages !== undefined) {
    let pageNext = getPageNext(response.pages?.next)
    while (pageNext) {
      response = await client.role.getMany({
        spaceId,
        query: { limit: pageLimit, pageNext }
      })
      allItems.push(...response.items)
      logEmitter.emit('info', `Fetched ${allItems.length} roles`)
      pageNext = getPageNext(response.pages?.next)
