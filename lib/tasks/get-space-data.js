...
// Guards against a single page request hanging indefinitely and blocking the
// whole export. Not a retry mechanism - the SDK client already retries
// transient failures. On timeout we log and optimistically move on to the
// next page rather than getting stuck forever.
...
export default function getFullSourceSpace({
  ...
  return new Listr([
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        return pagedGet((query) => client.contentType.getMany({ spaceId, environmentId, query }), { entityName: 'content types', timeoutMs: ENTRY_AND_CONTENT_TYPE_FETCH_TIMEOUT_MS })
          .then(extractItems)
          .then((items) => {
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        return pagedGet((query) => client.tag.getMany({ spaceId, environmentId, query }), { entityName: 'tags', timeoutMs: ENTRY_AND_CONTENT_TYPE_FETCH_TIMEOUT_MS * 3 })
          .then(extractItems)
          .then((items) => {
            ctx.data.tags = items
          })
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        ...
        return pagedGet(fetchFn, { query: queryEntries, entityName: 'entries', timeoutMs: ENTRY_AND_CONTENT_TYPE_FETCH_TIMEOUT_MS })
          .then(extractItems)
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        ...
        return pagedGet(fetchFn, { query: queryAssets, entityName: 'assets', timeoutMs: ASSET_FETCH_TIMEOUT_MS })
          .then(extractItems)
          .then((items) => filterDrafts(items, includeDrafts, cdaClient))
          .then((items) => filterArchived(items, includeArchived))
          .then((items) => removeTags(items, stripTags))
    ...
    },
    {
      ...
      task: wrapTask((ctx) => {
        return pagedGet((query) => client.locale.getMany({ spaceId, environmentId, query }), { entityName: 'locales', timeoutMs: ENTRY_AND_CONTENT_TYPE_FETCH_TIMEOUT_MS * 3 })
          .then(extractItems)
          .then((items) => {
 ...
 * @param options {{ query?: object, entityName?: string, timeoutMs?: number }}
 */
function pagedGet(fetchFn, { query: userQuery = null, entityName, timeoutMs } = {}, skip = 0, aggregatedResponse = null) {
  ...
  const pagePromise = timeoutMs
    ? withTimeout(fetchPage, { ms: timeoutMs, label: `Fetching ${entityName} (skip ${skip}, limit ${limit})` })
    : fetchPage()
  ...
  return pagePromise
    .then((response) => {
      ...
      return pagedGet(fetchFn, { query: userQuery, entityName, timeoutMs }, skip + response.items.length, aggregatedResponse)
    })
    .catch((err) => {
      ...
      return pagedGet(fetchFn, { query: userQuery, entityName, timeoutMs }, nextSkip, aggregatedResponse)
    })
}
