...
export default function parseOptions (params) {
  ...
  options.contentFile = options.contentFile || `contentful-export-${options.spaceId}-${options.startTime.format('YYYY-MM-DDTHH-mm-SS')}.json`
  ...
  if (options.query && options.query.length > 0) {
    const querystr = options.query.join('&')
    options.query = qs.parse(querystr)
  }
