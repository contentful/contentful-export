...
export default yargs
  ...
  .check(function (argv) {
    ...
    if (argv.proxy && !(proxySimpleExp.test(argv.proxy) || proxyAuthExp.test(argv.proxy))) {
      log.error('Please provide the proxy config in the following format:')
      log.error('host:port or user:password@host:port')
      log.error(argv.proxy)
      process.exit(1)
    }
    return true
  })
