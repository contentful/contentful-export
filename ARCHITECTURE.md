...
| `bin/contentful-export` | CLI entry point. Requires the built `dist/` output, prints deprecation notice pointing users to `contentful-cli`, then runs the export. |
...
- **contentful-cli** (`contentful space export` command) -- Primary CLI consumer; the standalone CLI in this repo is deprecated in favor of this
